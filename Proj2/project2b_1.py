...
#reconstruction
rz3 = T.nnet.sigmoid(T.dot(y3, W3.transpose()) + b3_prime)
rz2 = T.nnet.sigmoid(T.dot(rz3, W2.transpose()) + b2_prime)
rz1 = T.nnet.sigmoid(T.dot(rz2, W1.transpose()) + b1_prime)
...
#run through ffn
test = theano.function(inputs = [tilde_x], outputs = [rz1, y3])
...
pylab.savefig("project2b1_crossEntropy")
...
pylab.savefig('figure_2b1_hiddenLayer1.png')
...
pylab.savefig('figure_2b1_hiddenLayer2.png')
...
pylab.savefig('figure_2b1_hiddenLayer3.png')

ind = np.random.randint(low=0, high=2000)
pylab.gray()
for i in range(100):
    z, y = test(teX[ind+i:ind+1+i,:])
    pylab.figure("inputs")
    pylab.subplot(10, 10, i+1); pylab.axis('off'); pylab.imshow(teX[ind+i:ind+i+1, :].reshape(28, 28))
    pylab.figure("reconstructed")
    pylab.subplot(10, 10, i+1); pylab.axis('off'); pylab.imshow(z[0].reshape(28, 28))
    pylab.figure("hiddenLayerActivation")
    pylab.subplot(10, 10, i + 1); pylab.axis('off'); pylab.imshow(y[0].reshape(20, 20))
pylab.figure("inputs")
pylab.savefig("project2b1_inputImages")
pylab.figure("reconstructed")
pylab.savefig("project2b1_reconstructedImages")
pylab.figure("hiddenLayerActivation")
pylab.savefig("project2b1_hiddenLayerActivation")
